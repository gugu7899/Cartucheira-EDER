...
class Audio:
    ...
    def play(self,path:Path,volume):
        self.stop(); self.sound=pygame.mixer.Sound(str(path)); self.duration=self.sound.get_length(); self.samples=pygame.sndarray.array(self.sound)
        ...
        self.volume(volume); self.channel.play(self.sound)
    def fadeout(self,milliseconds=650):
        if self.sound:self.channel.fadeout(milliseconds)
    def stop(self):
        if hasattr(self,"channel"):self.channel.stop()
    ...
    def volume(self,value):
        ...
        if hasattr(self,"channel"):self.channel.set_volume(min(1.0,self.base_volume*self.gain))
